...
//DEFINICJE STALYCH
#define INSTANCJA_NUMER 1 // zmienna okreslajaca numer pliku wejsciowego i wyjsciowego

#define MAINTENANCE_FIRST_POCESSOR 15//LICZBA MAINTENENSOW

#define liczba_zadan 50 // liczba zadan w instancjji

#define liczba_tworzonych_rozwiazan 40 //ilosc iteracji petli korzystajacej z generatora losowego badz generatora macierzy w metaheurystyce

#define liczba_generowanych_losowo 20 // liczba rozwiazan tworzonych losowo przed wejsciem do g³ownej petli metaheurystyki
// macierz feromnowa po turnieju uzupelnia polowa z nich, zeby metaheurystyka miala na czym pracowac

#define wspolczynnik_odparowania 0.1 //wspolczynnik parowania macierzy feromnowej

#define WSPOLCZYNNIK_WYGLADZENIA 12  //wspolczynnik wygladzania macierzy feromnowej

int MAX_DURATION_PROGRAM_TIME =11; //maksymalny czas dzialania petli metaheurystyki


int probabilityOfRandomGeneration = 100; // startowa wartosc prawdopodobiensta skorzystania z generatora losowego

int o_ile_sie_zmienia = 2; // przechowuje informacje o ile maleje prawdopodbienstwo skorzystania z generatora losowego


int najlepsza_wartosc = INT_MAX; //przechowuje poczatkowa wartosc najlepszego rozwiazania ( najmniejszej funkcji celu )
...
int **tablica_maintenance_m1 = new int*[MAINTENANCE_FIRST_POCESSOR];
...
void zapiszdopliku(int rows, int columns, int **tablica)
{
    ofstream plik;
    string nazwa= "Testy/Instancja" + to_string(INSTANCJA_NUMER) + ".txt";
    CreateDirectory("Testy", NULL);
    plik.open(nazwa);
    if(plik.good())
    {
        plik<<"****"<<INSTANCJA_NUMER<<"****"<<"\n";
...
void dopiszdopliku(int rows, int columns, int **tablica)
{
    ofstream plik;
    string nazwa= "Testy/Instancja" + to_string(INSTANCJA_NUMER) + ".txt";
...
void dopisz_plikwynikowy(vector<vector<int>> wektor_wektorow,int indeksM1, int indeksM2, int najlepszy_czas, int czas_po_losowym,int suma_maintenaceM1, int suma_maintenaceM2,int sumaIDLE_M1, int sumaIDLE_M2)
{
    ofstream plik;
    string nazwa1= "Testy/Plik_wynikowy" + to_string(INSTANCJA_NUMER) + ".txt";
    plik.open(nazwa1);
    if(plik.good())
    {
        plik<<"****"<<INSTANCJA_NUMER<<"****"<<"\n";
...
{
    int tablicapomocnicza[MAINTENANCE_FIRST_POCESSOR];
    for(int j=0; j<MAINTENANCE_FIRST_POCESSOR; j++)
    {
        tablicapomocnicza[j]=tablica_maintenance_m1[j][1]-i;
    }
    int min_odl;
    int min_indeks=-1;
    for(int k=0; k<MAINTENANCE_FIRST_POCESSOR; k++) //jezeli jestem na maintanencie zwracam jego numer
        if(tablicapomocnicza[k]==0)
            return k;

    for(int k=0; k<MAINTENANCE_FIRST_POCESSOR; k++)
    {
        if(tablicapomocnicza[k]>0)
        {
    ...
    for(int j=min_indeks; j<MAINTENANCE_FIRST_POCESSOR; j++)
    {
...
int generator(int n,int **tablica_zadan, int ** tablica_maintenance_m1)   //n-liczba zadan ( globalna ) pozniej sie poprawi
{
    for(int i = 0; i < n; i++)
    {
    ...
    for(int i = 0; i < MAINTENANCE_FIRST_POCESSOR; i++)
    {
    ...
    {
        tablica_zadan[i][0] = (rand()%10)+1; //czas operacji 1
        tablica_zadan[i][1] = (rand()%40)+1; // czas operacji 2
        tablica_zadan[i][2] =  (rand()%n/4)+1; // ready time operacji 1 na maszynie 1
    }
    //druga polowa
    for(int i=n/2; i<n; i++)
    {
        tablica_zadan[i][0] = (rand()%40)+1; //czas operacji 1
        tablica_zadan[i][1] = (rand()%10)+1; // czas operacji 2
        tablica_zadan[i][2] =(rand()%n/4)+1; // ready time operacji 1 na maszynie 1
    }

    zapiszdopliku(n, 3, tablica_zadan);
    int dlugosc_op1m1=0;
    for (int i=0; i<n; i++)
    {
    ...
    int licz = 0;
    for(int i=0; i<MAINTENANCE_FIRST_POCESSOR;)
    {
    ...
    dopiszdopliku(MAINTENANCE_FIRST_POCESSOR,2,tablica_maintenance_m1);
...
//--------------- TURNIEJ  ----------------------------
//zostawia polowe z obecnych wektorow uszeregowan
void turniej ()
{
    int indeks_zad1;
    int indeks_zad2;
...
//ilosc wektorow=ilosc roznych uszeregowan
    int ilosc_wek_po_turnieju=(ilosc_wektorow+1)/4; //ilosc  uszeregowan jakie zostana po turnieju , dlugosc wektora to bedzie
...
void metaheurystyka()
{
    ...
    int liczba_mutacji;
    int j;
    double uzupelniajaca;
    double uzupelniajaca1;
    int b;
    int najlepsza_wartosc_losowego=INT_MAX; //najlepsza f celu jaka osiaga gen losowy (int_max - najwieksza wartosc inta)
    //petla dodajaca do obecnych wektorow uszeregowan, uszeregowania wygenerowanie przez generator
    for(int i=0; i<liczba_generowanych_losowo; i++)
    {
        generator_losowy();
    }

    turniej(); // zostawiam polowe z tworzonych poczatkowo losowo

    najlepsza_wartosc_losowego=best_funkcja_celu_wektora_wektorow(wektor_rozw); //zapisuje wartosc najlepszego z losowych uszeregowan po turnieju

cout<<"najlepsza wartosc losowego po turnieju " <<najlepsza_wartosc_losowego<<endl;
    ...
    int a = best_funkcja_celu_wektora_wektorow(wektor_rozw);
    cout<<"najlepsza funkcja celu wynosi " << a <<endl;
    best_funkcja_celu_wektora_wektorow_void(); //do wektora uszeregowan zapisuje  najlepszego uszeregowanie ( M1 i M2 )
int piter;
cin >> piter;
    int iteracja=0;
    while ((clock()-zegar_start)<MAX_DURATION_PROGRAM_TIME*CLOCKS_PER_SEC)   //max... dlugosc trwania jaka dopuszczam w globalnych (int w minutach)
    {
        ...
        for(int i=0; i<liczba_tworzonych_rozwiazan; i++)
        {
            ...
            if(iteracja%3==0) probabilityOfRandomGeneration-=o_ile_sie_zmienia; //to zesmy dodali (zmien tej wartosci co 3 iteracje)
            if((rand()+1.0) < RAND_MAX * probabilityOfRandomGeneration/100)   // co 3 iteracje prawdopodobienstwo uzycia losowego maleje o 2%
            {
        ...
        for(int i=0; i<liczba_mutacji; i++)
        {
         ...
         b=best_funkcja_celu_wektora_wektorow(wektor_rozw);
        cout << "Po mutacji najlepsza wartosc to  " <<b<<endl;
        turniej();
        ...
        //uzupelnianie macierzy feromonowej nowymi rozwiazaniami po mutacji, i 2gim turnieju
        for(int i=0; i<pomocniczyrozmiar; i++)
        {
            if(i%2==0)
        {
            value1=funkcja_celu(wektor_rozw[i])+funkcja_celu(wektor_rozw[i+1]); //parametr do wartosci wypelniania macierzy zalezy od funkcji celu maszyny 1 danego uszeregowania
        }
//        uzupelniajaca=((double)najlepsza_wartosc_losowego / (double)value);
//        wypelnianie_macierzy_feromonowej__uszeregowania(wektor_rozw[i], uzupelniajaca, i%2 );
             uzupelniajaca1=((double)najlepsza_wartosc_losowego / (double)value1);
            wypelnianie_macierzy_feromonowej__uszeregowania(wektor_rozw[i], uzupelniajaca1);
        }
        ...
        cout<<"--------- ZAKONCZYLEM GLOWNA PETLE METAHEURYSTYKI ------------"<<endl;
    }
    ...
    dopisz_plikwynikowy(najlepszy_wektor_rozwiazan,m1,m2,najlepsza_wartosc,najlepsza_wartosc_losowego,suma_maint,0,suma_przerw_M1,suma_przerw_M2);
